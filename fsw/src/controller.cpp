...
// vertical speed, a vertical velocity reference below kHoverVsp, roll and pitch within kHoverLevel), clamped to its
// parameter range.
...
ControlRequest Controller::run(const Reference& ref, const State& nav, Mode mode, float dt, std::uint8_t profile) {
    ...
    const bool climb_ref = std::fabs(v_ref.z) >= kHoverVsp;
