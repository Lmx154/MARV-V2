...
// baro 50 Hz, GNSS 10 Hz from t = 1 ms): 2 s at rest yawed +90 deg (with the ground-contact transient on the first
// IMU sample), then translation on all three axes and rotation about all three, each A (1 - cos(w t)) so velocity
// and body rate start from zero. The same trajectory as tests/test_eskf.cpp. Conventions of contracts.hpp: NED / FRD,
// specific force R^T (a - g). The truth and the synthesis are double; the estimators see the float SensorBus only.
#pragma once
...
inline Truth truth_at(double t) {
    Truth s{};
    s.t = t;
    ...
    for (int i = 0; i < 3; ++i) {
        ...
        s.a[i] = t > kRest ? A[i] * W[i] * W[i] * std::cos(W[i] * tm) : 0.0;
    }
...
public:
    ...
    bool next(SensorBus& bus, Truth& s) {
        ...
        if (k_ % 10 == 0) {
            bus.fresh |= kMag;
            bus.mag.field_frd_ut = body_of(R, kMagNed, !o_.flip_mag);
        }
        if (k_ % 20 == 0) {
            bus.fresh |= kBaro;
