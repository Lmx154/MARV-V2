...
launches the sim (an airframe of `sitl/airframes`, wind/gusts/location, firmware on this computer or on the connected flight controller, the Gazebo window) holding the rig lock.
On the flight controller with no sim: `scripts/gcs.sh --serial /dev/serial/by-id/usb-MARV_MARV_flight_controller_*` (its Flash button takes the lock).
